#include <math.h>   // smallpsmlt - paper 0185
#include <stdlib.h> // derived from smallpt, a path tracer by Kevin Beason, 2008
#include <stdio.h>  // Usage: ./smallpsmlt time_sec
...
#define MaxPathLength 13
#define Glossiness 100.0
#define PixelWidth 640	// Image width
#define PixelHeight 480	// Image height
#define N_Init 10000
...
// compensated summation, reduces numerical errors
struct TKahanAdder {
	double sum, carry, y; 
	TKahanAdder (
		const double b = 0.0) {
...
// path data
struct Vert {
	Vec p, n;		// Position og Normal
	int id; 		// sphere id
	Vert(){}; 
	Vert(Vec p_, Vec n_, int id_) : p(p_), n(n_), id(id_) {}
};
struct Path {
	Vert x[MaxEvents];	// Points in path
...
struct PathContribution {
	Contribution c[MaxEvents*MaxEvents];	// Array of contribution elements
	int n; 									// Amount of elements
	double sc; 								// Scalar contribution
	PathContribution(){
		n = 0; 
		sc = 0.0;
	}
...
// Add all color from a PathContribution to a pixel from the PathContribution
// Runs through the whole path and adds colors to pixels
...
struct TMarkovChain { 
	...
	TMarkovChain large_step() const {
		TMarkovChain Result;
		Result.C = (*this).C;
		for (int i = 0; i < NumStates; i++) Result.u[i] = rndmlt();
		return Result;
	}
...
inline double GeometryTerm(const Vert e0, const Vert e1) {
	const Vec dv = e1.p - e0.p;								// Distance between vertices
	const double d2 = dv.dot(dv);							// Distance squared
	return fabs(e0.n.dot(dv) * e1.n.dot(dv)) / (d2 * d2);	// (dot(normal, dv) / d2) twice
}

// TODO: Fat den her, også
inline double DirectionToArea(const Vert current, const Vert next) {
	const Vec dv = next.p - current.p;						// Distance between vertices
	const double d2 = dv.dot(dv);							// Distance squared
	return fabs(next.n.dot(dv)) / (d2 * sqrt(d2));			// dot product of next normal and distance divided by d^3
}
...
Vec PathThroughput(const Path Xb) {
	...
	for (int i = 0; i < Xb.n; i++) {    // Path size
		if (i == 0) {					// First
			double W = 1.0 / double(PixelWidth * PixelHeight);
			Vec d0 = Xb.x[1].p - Xb.x[0].p;		// Distance
			const double dist2 = d0.dot(d0);	// Distance squared
			d0 = d0 * (1.0 / sqrt(dist2));		// Distance divided by distance squared
			const double c = d0.dot(Camera.w);
			const double ds2 = (Camera.dist / c) * (Camera.dist / c);
			W = W / (c / ds2);
			f = f * (W * fabs(d0.dot(Xb.x[1].n) / dist2));
		} else if (i == (Xb.n - 1)) {	// Last
			if (sph[Xb.x[i].id].refl == LGHT) { // Must be a light
...
bool isConnectable(const Path Xeye, const Path Xlight, double &px, double &py) {
	...
	if ( (Xeye.n == 0) && (Xlight.n >= 2) ) {
		...
		// light tracing
		Ray r(Xeye.x[0].p, (Xlight_e.p - Xeye.x[0].p).norm());	// Ray from eye towards end of light path
		double t; 
		int id; 
		Result = (intersect(r, t, id)) && (id == Xlight_e.id);	// Hits end of light path
		Direction = r.d;
	} else {
		// shadow ray connection
		Ray r(Xeye_e.p, (Xlight_e.p - Xeye_e.p).norm());		// Ray from end of eye path to end of light path
		double t; 
		int id;
		Result = (intersect(r, t, id)) && (id == Xlight_e.id);	// Hits end of light path
		Direction = (Xeye.x[1].p - Xeye.x[0].p).norm();			// First ray trace direction
	}
	...
	for (int NumEyeVertices = 0; NumEyeVertices <= PathLength + 1; NumEyeVertices++) {
		...
		for (int i = -1; i <= NumEyeVertices - 2; i++) {
			if (i == -1) {
				...
				p = p * 1.0;
			} else if (i == 0) {  // First hit?
				p = p * 1.0 / double(PixelWidth * PixelHeight);						// divided by image size
				Vec Direction0 = (SampledPath.x[1].p - SampledPath.x[0].p).norm();	// Direction from first to second point
				double CosTheta = Direction0.dot(Camera.w);							// Cosine of angle from camera
				double DistanceToScreen2 = Camera.dist / CosTheta;					// Distance to screen
				DistanceToScreen2 = DistanceToScreen2 * DistanceToScreen2;			// Distance to screen squared
				p = p / (CosTheta / DistanceToScreen2);								// Divided by cosine of angle divided by distance to screen squared				
...
void TracePath(Path &path, const Ray r, const int RayLevel, const int MaxRayLevel) {
	...
	const Sphere &obj = sph[id]; 		// The hit sphere
	Vec p = r.o + r.d*t,				// The hit point
	n = (p - obj.p).norm();				// The hit point normal
...
PathContribution CombinePaths(const Path EyePath, const Path LightPath, const int SpecifiedNumEyeVertices = -1, const int SpecifiedNumLightVertices = -1) {
	...
	for (int PathLength = MinPathLength; PathLength <= MaxPathLength; PathLength++) {
		for (int NumEyeVertices = 0; NumEyeVertices <= PathLength + 1; NumEyeVertices++) {
			const int NumLightVertices = (PathLength + 1) - NumEyeVertices;	// 0
...
int smallpsmlt(int argc, char *argv[]) {
	unsigned long samps = 0;
	const int ltime = (argc == 2) ? MAX(atoi(argv[1]),0) : 60;
	...
	for (;;) {
		samps++;
		fprintf(stderr, "\rPSMLT %c", progr[(samps>>8)&3]);

		// sample the path
		double isLargeStepDone;
		if (rndmlt() <= LargeStepProb) {
		...
		// accumulate samples
		if (proposal.C.sc > 0.0) AccumulatePathContribution(proposal.C, (a + isLargeStepDone)/
															(proposal.C.sc/b + isLargeStepDone));
		if (current.C.sc > 0.0) AccumulatePathContribution(current.C, 
			(1.0 - a)/
			(current.C.sc/b + isLargeStepDone));

		// update the chain
		if (rndmlt() <= a) current = proposal;
	}
